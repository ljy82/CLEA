...
class DBP100KattNeighbors():
    ...
    def id_neighbors_loader(self):
        data = load_list(join(self.path, "ent_attrs_" + self.doc_id ))
        ...
        for alist in data:
            if not (int(alist[0])+1) in self.id_neighbors_dict.keys():
                self.id_neighbors_dict[int(alist[0])+1] = [self.id_entity[int(alist[0])][0]]
                self.id_attr_dict[int(alist[0])+1] = [int(alist[0])+1]
            for str in alist[1:]:
                if int(str)+1 in self.attr_reinfo.keys():
                    self.attr_reinfo[int(str)+1] = self.attr_reinfo[int(str)+1] + 1
                if int(str) in self.id_attr.keys():
                    self.id_neighbors_dict[int(alist[0])+1].append(self.id_attr[int(str)][0])
                    self.id_attr_dict[int(alist[0])+1].append(int(str)+1)
        ...
        self.attr_reinfo = (self.attr_reinfo.max() - self.attr_reinfo) / (
                    self.attr_reinfo.max() - self.attr_reinfo.mean())
